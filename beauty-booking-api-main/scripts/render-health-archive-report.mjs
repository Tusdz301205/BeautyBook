...
const r=JSON.parse(readFileSync('../tmp/manager-refactor/health-archive-rehearsal.json','utf8'));
const p=JSON.parse(readFileSync('../tmp/manager-refactor/health-archive-preflight.json','utf8'));
const v=JSON.parse(readFileSync('../tmp/manager-refactor/health-archive-final-verification.json','utf8'));
const md=['# BeautyBook — Kết quả rehearsal archive health/consultation', '',
  'Ngày 19/09/2026. **REHEARSAL ĐẠT — CHỜ NGƯỜI DÙNG DUYỆT. CHƯA APPLY DATABASE CHÍNH. CHƯA VẼ SƠ ĐỒ FINAL.**', '',
  `Copy kiểm chứng: \`${r.database}\`. Copy restore backup: \`${r.restoreDatabase}\`. Preflight gốc lấy từ copy sạch 2026091901; copy 1903 được restore từ backup pre-archive đó, không lấy từ bản đã có fixture lỗi.`, '',
...
for(const row of r.rows) {
  const old=p.rows.find(x=>x.model===row.model);
  const incoming=old.fks.filter(f=>f.target===row.table).length,outgoing=old.fks.filter(f=>f.source===row.table).length;
  md.push(`| ${row.model} / \`${row.table}\` | ${row.old} | ${row.archived} | Giữ ${incoming} FK vào, ${outgoing} FK ra, cùng constraint/OID/actions; tham chiếu tự chuyển theo OID | Bỏ model và inverse relation khỏi Prisma; không có production delegate/route cần thay bằng fake data | Fingerprint/PK/FK/trigger khớp; integration đạt | Chuyển cùng table về public bằng reverse recipe; hoặc restore backup đã thử |`);
}
md.push('', 'Chi tiết tên FK, source line và mapping trước migration: [HEALTH_ARCHIVE_PREFLIGHT.md](HEALTH_ARCHIVE_PREFLIGHT.md).', '',
  ...
  `- ${r.preservedTables} bảng có trước giữ count/fingerprint toàn bộ tại thời điểm migration; kiểm trước khi thêm fixture test. Bao gồm 4.000 booking, customer, staff, business, payment, review và history.`,
  `- ${r.beforeFKs}/${r.afterFKs} FK trước/sau: zero orphan. PK/FK/index/column type/original trigger cùng OID và definition metadata. Hai consent nguyên vẹn, không chèn bản thay thế.`,
  ...
  `- Check cuối READ ONLY lúc ${v.checkedAt}: DB chính còn ${v.mainBookings} booking, đủ 12 public health tables, 2 consent có fingerprint không đổi, 0/5 migration mới applied. Archive sau tests cũng không đổi.`, '',
  '## 4. Kiểm thử', '',
  '| Kiểm tra | Kết quả |', '|---|---|',
...
writeFileSync('../docs/HEALTH_ARCHIVE_REHEARSAL_RESULT.md',md.join('\n'));
console.log('Wrote HEALTH_ARCHIVE_REHEARSAL_RESULT.md');
