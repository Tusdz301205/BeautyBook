...
import { branchesApi, businessApi, financeOperationsApi, impactApi, loyaltyApi, ownershipApi, waitlistApi } from '../../api/apiClient';
...
const panels = [
  ...
  ['cash', 'Tiền mặt & hóa đơn', Banknote], ['loyalty', 'Điểm thưởng', Award],
...
export default function SalonOperations() {
  ...
  const [data, setData] = useState({ impacts: [], waitlist: [], shifts: [], invoices: [], invoiceRequests: [], loyalty: null, transfers: [], versions: null });
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const [busy, setBusy] = useState('');

  const load = useCallback(async (preferredBranchId) => {
    setLoading(true); setError('');
    try {
      const branches = await branchesApi.getAccessible();
      const branchId = preferredBranchId || context.branchId || branches?.[0]?.id || '';
      let business = null;
      if (can('business:update:tenant')) try { business = await businessApi.getMyOnboarding(); } catch { business = null; }
      const businessId = business?.id || branches?.[0]?.businessId || branches?.[0]?.business?.id || '';
      setContext({ businessId, branchId, branches: branches || [] });
      const [impacts, waitlist, shifts, invoices, invoiceRequests, loyalty, transfers, versions] = await Promise.all([
        can('booking:read:tenant') || can('booking:read:branch') ? impactApi.list() : [],
        branchId && (can('booking:read:tenant') || can('booking:read:branch')) ? waitlistApi.branch(branchId) : [],
        branchId && (can('payment:read:tenant') || can('payment:read:branch')) ? financeOperationsApi.shifts(branchId) : [],
        can('payment:read:tenant') || can('payment:read:branch') ? financeOperationsApi.invoices() : [],
        can('payment:read:tenant') || can('payment:read:branch') ? financeOperationsApi.invoiceRequests() : [],
        businessId && can('promotion:manage:tenant') ? loyaltyApi.liability(businessId) : null,
        businessId && can('business:update:tenant') ? ownershipApi.list(businessId) : [],
        businessId && can('business:update:tenant') ? ownershipApi.versions(businessId) : null,
      ]);
      setData({ impacts, waitlist, shifts, invoices, invoiceRequests, loyalty, transfers, versions });
    } catch (requestError) { setError(requestError.message || 'Không thể tải trung tâm vận hành'); }
    finally { setLoading(false); }
  }, [can, context.branchId]);
  useEffect(() => { void load(); }, []);
  ...
  const visible = panels.filter(([key]) => key === 'ownership' ? can('business:update:tenant') : key === 'loyalty' ? can('promotion:manage:tenant') : key === 'cash' ? can('payment:read:tenant') || can('payment:read:branch') : can('booking:read:tenant') || can('booking:read:branch'));

  return <Page><PageHeader eyebrow="Điều phối" title="Trung tâm vận hành" description="Xử lý lịch bị ảnh hưởng, waitlist, ca tiền mặt, hóa đơn, điểm thưởng và thay đổi pháp lý trên dữ liệu thật." actions={<Button variant="secondary" loading={loading} onClick={() => load()}><RefreshCw size={16} />Làm mới</Button>} />
    {context.branches.length > 1 && <Field label="Chi nhánh"><Select value={context.branchId} onChange={(event) => load(event.target.value)}>{context.branches.map((branch) => <option key={branch.id} value={branch.id}>{branch.name}</option>)}</Select></Field>}
    ...
    {error ? <Card><ErrorState message={error} onRetry={() => load()} /></Card> : loading ? <Card className="p-5"><Skeleton rows={8} /></Card> : <>
      {tab === 'impact' && <ImpactPanel rows={data.impacts} selectedId={selectedImpactId} busy={busy} run={run} />}
      {tab === 'waitlist' && <WaitlistPanel rows={data.waitlist} busy={busy} run={run} />}
      {tab === 'cash' && <CashInvoicePanel branchId={context.branchId} shifts={data.shifts} invoices={data.invoices} invoiceRequests={data.invoiceRequests} busy={busy} run={run} />}
      {tab === 'loyalty' && <LoyaltyPanel businessId={context.businessId} liability={data.loyalty} busy={busy} run={run} />}
      {tab === 'ownership' && <OwnershipPanel businessId={context.businessId} rows={data.transfers} versions={data.versions} busy={busy} run={run} />}
    </>}
  </Page>;
}
...
function CashInvoicePanel({ branchId, shifts, invoices, invoiceRequests, busy, run }) {
  ...
  return <div className="space-y-5">
    <div className="grid gap-5 lg:grid-cols-2">
      <Card className="p-5"><h2 className="font-bold">{open ? 'Đóng và đối soát ca' : 'Mở ca tiền mặt'}</h2>{open ? <form className="mt-4 space-y-3" onSubmit={(event) => { event.preventDefault(); run(open.id, () => financeOperationsApi.closeShift(open.id, { actualClosingBalance: Number(closing.amount), varianceReason: closing.reason, approvalThreshold: 100000 }), 'Đã đóng ca'); }}><p className="text-sm">Đầu ca {money(open.openingBalance)} · {stamp(open.openedAt)}</p><Field label="Tiền thực tế cuối ca" required><Input type="number" min="0" value={closing.amount} onChange={(event) => setClosing((current) => ({ ...current, amount: event.target.value }))} required /></Field><Field label="Lý do nếu chênh"><Input value={closing.reason} onChange={(event) => setClosing((current) => ({ ...current, reason: event.target.value }))} /></Field><Button type="submit" loading={busy === open.id}>Đóng ca</Button></form> : <form className="mt-4" onSubmit={(event) => { event.preventDefault(); run('open', () => financeOperationsApi.openShift(branchId, Number(opening)), 'Đã mở ca'); }}><Field label="Số dư đầu ca" required><Input type="number" min="0" value={opening} onChange={(event) => setOpening(event.target.value)} required /></Field><Button type="submit" loading={busy === 'open'}>Mở ca</Button></form>}</Card>
